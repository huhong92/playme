...
class Json
{
	...
	public function output($message, $charset = null) {
		header('content-Type: application/json; charset='.(empty($charset) ? 'UTF-8' : $charset), true);
		echo $this->encode($message);
		exit;
	}
	
	public function output_jsonp($jsonp,$message, $charset = null) {
		header('content-Type: application/json; charset='.(empty($charset) ? 'UTF-8' : $charset), true);
		echo $jsonp.'('.$this->encode($message).')';
		exit;
	}
}
